...
public class ResourceChecker : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("Export Material")) Export();
        isKeepFolder = GUILayout.Toggle(isKeepFolder, "Keep Folder");
    ...
    void ListMaterials()
    {
        ...
        foreach (MaterialDetails tDetails in ActiveMaterials)
        {
            if (tDetails.material != null)
            {
                ...
                GUILayout.Label(shaderLabel, GUILayout.Width(150));

                ////TODO 材质替换
                //Material newMat = (Material)EditorGUILayout.ObjectField("", null, typeof(Material), false, GUILayout.Width(200));
    ...
    private void Export()
    {
        folder = EditorUtility.SaveFolderPanel("Save textures to directory", "", "");
        if (folder.Length == 0) return;

        if (ActiveMaterials.Count == 0) return;
        for (int i = 0; i < ActiveMaterials.Count; i++)
        {
            var detail = ActiveMaterials[i];
            GetAbsoulatePath(detail.material);
        }
        if (ActiveTextures.Count == 0) return;
        for (int i = 0; i < ActiveTextures.Count; i++)
        {
            var detail = ActiveTextures[i];
            GetAbsoulatePath(detail.texture);
        }
    }
    ...
    private void GetAbsoulatePath(Object obj)
    {
        //获取文件路径
        string projectPath = Application.dataPath.Replace("Assets", "");
        string assetPath = AssetDatabase.GetAssetPath(obj);
        string destPath = Path.Combine(folder, assetPath);
        ...
        if (isKeepFolder)
        {
            ...
            File.Copy(projectPath + assetPath, Path.Combine(destFolder, fileName), true);
        }
        else
        {
            File.Copy(Path.Combine(projectPath, assetPath), Path.Combine(folder, fileName));
        }
    }
    #endregion
}
